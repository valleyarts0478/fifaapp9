...
<section class="text-gray-600 body-font relative">
  <div class="container px-5 py-24 mx-auto">
    <div class="flex flex-col text-center w-full mb-12">
      <h1 class="sm:text-3xl text-2xl font-medium title-font mb-4 text-gray-900">チーム登録</h1>
    </div>
    <div class="lg:w-1/2 md:w-2/3 mx-auto">
        <!-- Validation Errors -->
        <x-auth-validation-errors class="mb-4" :errors="$errors" />
        <x-flash-message status="session('status')" />
        <form method="post" action="{{ route('admin.team_owners.store') }}" enctype="multipart/form-data">
            @csrf
      <div class="-m-2">
        ...
        <div class="p-2 w-1/2 mx-auto">
          <div class="relative">
            <label for="league_id" class="leading-7 text-sm text-gray-600">リーグ名</label>
              <select name="league_id" id="league_id" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
        ...
        <div class="p-2 w-1/2 mx-auto">
          <div class="relative">
            ...
            <input type="text" id="team_name" name="team_name" value="{{ old('team_name') }}" required class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
          </div>
        </div>
        <div class="p-2 w-1/2 mx-auto">
          <div class="relative">
            <label for="team_abb" class="leading-7 text-sm text-gray-600">チーム略称</label>
            <input type="text" id="team_abb" name="team_abb" value="{{ old('team_abb') }}" required class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
          </div>
        </div>
