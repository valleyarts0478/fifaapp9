    ...
    <div class="container px-5 py-24 mx-auto">
      <div class="flex items-center lg:w-3/5 mx-auto border-b pb-10 mb-10 border-gray-200 sm:flex-row flex-col">
        ...
        <div class="flex-grow sm:text-left text-center mt-6 sm:mt-0">
          ...
          <div class="sm:text-right text-sm text-ral-200 font-semibold mx-auto">OG:{{$home_game->game_results->home_own_goal}}</div>
          @endif
          <div class="sm:text-right mx-auto">HOME</div>
          ...
          <p class="mb-2 border-ral-100 border-solid border-b-2 text-xl">{{$home->player_name}}</p>
            <div class="flex mb-2 leading-relaxed text-base">
              <div class="w-1/2 text-lg">
                @if($home->goals === NULL)
                <span class="opacity-20">G:{{$home->goals = 0}}</span>
                @elseif($home->goals >= 0)
                G:{{$home->goals}}
                @endif
              </div>
              <div class="w-1/2 text-lg">
                ...
                <span class="opacity-20">A:{{$home->assists = 0}}</span>
      ...
      <div class="flex items-center lg:w-3/5 mx-auto border-b pb-10 mb-10 border-gray-200 sm:flex-row flex-col">
        <div class="flex-grow sm:text-left  mt-6 text-center sm:mt-0">
          ...
          <p class="mb-2 border-ral-300 border-solid border-b-2 text-xl">{{$away->player_name}}</p>
            <div class="flex mb-2 leading-relaxed text-base">
              <div class="w-1/2 text-lg">
                @if($away->goals === NULL)
                <span class="opacity-20">G:{{$away->goals = 0}}</span>
                @elseif($away->goals >= 0)
                G:{{$away->goals}}
                @endif
              </div>
              <div class="w-1/2 text-lg">
                ...
                <span class="opacity-20">A:{{$away->assists = 0}}</span>
