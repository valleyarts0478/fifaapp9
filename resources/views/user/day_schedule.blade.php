        ...
        <div class="container max-w-4xl px-2 py-4 mx-auto md:px-8">
          ...
          <h2 class="mx-auto p-4 text-xl font-bold text-center">日程表</h2>
            ...
            <div class="text-center text-sm">第<span class="text-xl px-1">{{$game->section}}</span>節</div>
                ...
                <div class="w-20 flex flex-col text-center items-center">
                  <div class="w-12 h-12 inline-flex items-center justify-center rounded-full mb-2 flex-shrink-0">
                    @foreach ($team_names as $team_name)
                       ...
                       <img class="w-12 h-12" src="{{ asset('storage/teams/logo/' . $team_name->team_logo_url) }}" alt="team_logo">
                       @endif
                    @endforeach
                  </div>
                  <div class="flex-grow">
             ...
             @else
              <div class="flex flex-wrap border rounded-md pt-8 px-2 pb-2 mb-4">
               <div class="w-20 flex flex-col text-center items-center">
                 <div class="w-12 h-12 inline-flex items-center justify-center rounded-full mb-2 flex-shrink-0">
                   @foreach ($team_names as $team_name)
                      @if($team_name->team_name === $game->home_team)
              ...
              <div class="w-full">
               <div class="w-16 mx-auto my-4 rounded-full border border-gray-400">
                <button onclick="location.href='{{ route('user.day.schedule_show', ['team' => $game->id ]) }}'" class="text-gray-500 text-sm pt-1 pb-1 pl-2 pr-2 text-center">Details</button>    
               </div>
              </div>
              </div>
              @endif
