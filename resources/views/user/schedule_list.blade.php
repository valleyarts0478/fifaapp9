                ...
                <div class="container px-2 md:px-4 mx-auto">
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-4 xl:gap-4">
                        ...
                        @foreach ($team_list as $date => $teams)
                            @php
                                // section番号を取得
                                $sectionNumber = $section[$date]['section'] ?? null;
                                // dd($sectionNumber); // ここで null になっていないか確認
                            @endphp
                            @if ($sectionNumber)
                                <a href="{{ route('user.day.schedule', ['section' => $sectionNumber]) }}"
                                    ...
                                    <img class="w-full h-full object-cover object-center absolute inset-0 group-hover:scale-110 transition duration-200"
                                        src="{{ asset('storage/teams/' . 'ral_card1.jpg') }}" alt="team_logo">
                                    <div
                                        class="bg-gradient-to-r from-ral-400 md:via-transparent to-transparent absolute inset-0 pointer-events-none">
                                    </div>
                ...
                <div class="container px-2 md:px-8 mx-auto">
                    <div class="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-4 xl:gap-4">
                        ...
                        @foreach ($team_list_second as $date => $teams)
                            @php
                                $sectionNumber2 = $section2[$date]['section'] ?? null;
                            @endphp
                            @if ($sectionNumber2)
                                <a href="{{ route('user.day.schedule2', ['section' => $sectionNumber2]) }}"
                                    class="group h-48 md:h-64 xl:h-96 flex flex-col bg-gray-100 rounded-lg shadow-lg overflow-hidden relative">
                                    <img class="w-full h-full object-cover object-center absolute inset-0 group-hover:scale-110 transition duration-200"
                                        src="{{ asset('storage/teams/' . 'ral_card1.jpg') }}" alt="team_logo">
                                    <div
